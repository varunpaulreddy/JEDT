...
export function generateCMAPSSHealthData(engineId: string): CMAPSSHealthData {
  ...
  if (!engineId) {
    return {
      ...
      maintenanceRecommendation: 'Engine ID not provided - unable to generate health data',
      criticalComponents: []
    };
  }
  ...
  const engine = cmapssEngines.find(e => e.engineId === engineId);
  if (!engine) {
    // Return default health data instead of throwing error
    return {
      engineId,
      currentCycle: 0,
      remainingUsefulLife: 0,
      ...
      degradationRate: 0.2,
      faultProbability: 0.5,
      maintenanceRecommendation: `Engine ${engineId} not found in CMAPSS database`,
      criticalComponents: []
    };
  }
...
export const generateHistoricalData = (engineId: string, days: number) => {
  ...
  return sensorData.map((reading, index) => ({
    ...
    thrustOutput: Math.round(24000 + Math.random() * 1000),
