...
export default function EngineHealthScore({ engine }: EngineHealthScoreProps) {
  ...
  // Safe date handling with proper type checking
  ...
  const safeToLocaleDateString = (date: Date | string | undefined): string => {
    if (!date) return 'N/A';
  ...
  const nextMaintenanceTime = safeGetTime(engine.nextMaintenance);
  const lastMaintenanceTime = safeGetTime(engine.lastMaintenance);
  const currentTime = new Date().getTime();

  const daysUntilMaintenance = Math.ceil(
    (nextMaintenanceTime - currentTime) / (1000 * 60 * 60 * 24)
  );
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-xs">
            <div>
              ...
              <div>{safeToLocaleDateString(engine.lastMaintenance)}</div>
            </div>
            ...
            <div className="text-right">
              ...
              <div>{safeToLocaleDateString(engine.nextMaintenance)}</div>
            </div>
          </div>
              ...
              style={{ 
                width: `${Math.min(100, Math.max(0, 
                  ((currentTime - lastMaintenanceTime) / 
                   (nextMaintenanceTime - lastMaintenanceTime)) * 100
                ))}%` 
              }}
            ></div>
          </div>
